...
{
  public class ShaftDirectionVisualizer : BaseComponent, IAwakableComponent, IUpdatableComponent, IPreviewStateListener, IUnfinishedStateListener, IFinishedStateListener
  {
    ...
    {
      _meshRenderer.enabled = false;
      EnableComponent(); // Timberborn's native way to turn on the Update loop
    }
    ...
    {
      _meshRenderer.enabled = false;
      DisableComponent(); // Timberborn's native way to detach from the CPU loop
    }
